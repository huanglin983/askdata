...
| `/metrics/atomic` | `atomic_list` | 原子列表（Tab 可切换类型）；过滤/SQL/阶段汇率/维度绑定 |
| `/metrics/derived` | `derived_list` | 派生列表；叠加过滤；继承原子汇率与分类 |
| `/metrics/composite` | `composite_list` | 复合列表；公式 + 子指标 + 校验开关 |
| `/metrics/map` | `metric_map_view` | 指标依赖地图（复合→派生/原子）与孤立指标 |
| `/ask` | `ask` | 意图 → SQL → 执行 → 中文审计 |

列表页 UI：共享 `_metric_tabs.html`；表格使用 `table.compact`（固定紧凑行高 + 超长省略，悬停 `title` 看全文）。
