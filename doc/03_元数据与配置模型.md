...
|---|---|
| id / name | 标识与中文名称 |
| name_en | 英文名称（SQL 别名 / 列表第二行标识，如 `pj_total_cost`） |
| aliases | 同义别名（逗号分隔；列表「指标」列以标签展示，可检索） |
...
| `/metrics/atomic` | `atomic_list` | 原子列表；「指标」列叠排：名称 / 英文标识 / 别名标签；`?q=` 关键词；行内「创建派生」「地图」 |
| `/metrics/derived` | `derived_list` | 派生列表；同上叠排形态（标识为 ID）；关键词；行内「创建复合」；可 `?from_atomic=` 预填新建 |
| `/metrics/composite` | `composite_list` | 复合列表；同上叠排形态；关键词（含公式/子指标 ID）；可 `?from_derived=` 预填新建；行内「地图」 |
